...
int lockState PROGMEM ;
int lockPassword PROGMEM ;
...
void setup(){
  ...
  server.onNotFound([] () {
    server.send(400, "text/plain", F("Error! We don't have what you're looking for."));
  });
}
...
void setLockPassword(int newPassword) {
  if (String(abs(newPassword)).length() >= 5) {
    ...
    EEPROM.commit();
    server.send(200, F("application/json"), F("{\"success\": \"Password Changed\"}"));
    debugln("Password Changed.");
  } else {
    server.send(400, F("application/json"), F("{\"error\": \"Invalid Password\"}"));
...
void setLockState(int state) {
  ...
  if (state >= 0 && state <= 2) {
    ...
    server.send(200, F("application/json"), F("{\"success\": \"state changed\" }"));
  } else {
    server.send(400, F("application/json"), F("{\"error\": \"Argument out of bounds\"}"));
...
void getLockState() {
  ...
  server.send(200, F("application/json"), F("{\"state\": " + String(lockState) + "}"));
}

void getLockPassword() {
  debugln("Got lockpassword");
  server.send(200, F("application/json"), F("{\"password\": " + String(lockPassword) + "}"));
}
...
void handlePostRequest() {
  ...
  if (server.hasArg("plain")) {
    ...
    server.sendHeader(F("Access-Control-Allow-Methods"), F("POST"));

    DynamicJsonDocument doc(256);  // You can increase this size if needed
    ...
    if (doc.containsKey("action") && doc.containsKey("argument")) {  // Access the JSON object fields
      ...
      if (action == "setLockState") {
        ...
        server.send(400, F("application/json"), F("{\"error\": \"Invalid Action\"}"));
    ...
    server.send(400, F("application/json"), F("{\"error\": \"No data received\"}"));
  }
}
